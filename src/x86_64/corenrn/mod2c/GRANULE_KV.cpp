/*********************************************************
Model Name      : GRANULE_KV
Filename        : GRANULE_KV.mod
NMODL Version   : 7.7.0
Vectorized      : true
Threadsafe      : true
...
namespace coreneuron {
    ...
    static_assert(std::is_trivially_copy_constructible_v<GRANULE_KV_Store>);
    static_assert(std::is_trivially_move_constructible_v<GRANULE_KV_Store>);
    static_assert(std::is_trivially_copy_assignable_v<GRANULE_KV_Store>);
    static_assert(std::is_trivially_move_assignable_v<GRANULE_KV_Store>);
    static_assert(std::is_trivially_destructible_v<GRANULE_KV_Store>);
    static GRANULE_KV_Store GRANULE_KV_global;


    /** all mechanism instance variables and global variables */
    struct GRANULE_KV_Instance  {
        ...
        double* ion_ik{};
        double* ion_dikdv{};
        GRANULE_KV_Store* global{&GRANULE_KV_global};
    };


    /** connect global (scalar) variables to hoc -- */
    static DoubScal hoc_scalar_double[] = {
        {"usetable_GRANULE_KV", &GRANULE_KV_global.usetable},
        {nullptr, nullptr}
    };
    ...
    static inline int get_mech_type() {
        return GRANULE_KV_global.mech_type;
    }
    ...
    static void nrn_private_constructor_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new GRANULE_KV_Instance{};
        assert(inst->global == &GRANULE_KV_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(GRANULE_KV_Store);
    }

    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_KV_Instance const* inst);
    static inline void delete_instance_from_device(GRANULE_KV_Instance* inst);
    // Deallocate the instance structure
    static void nrn_private_destructor_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<GRANULE_KV_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &GRANULE_KV_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(GRANULE_KV_Store));
    ...
    static inline void setup_instance(NrnThread* nt, Memb_list* ml) {
        ...
        inst->gbar = ml->data+6*pnodecount;
        inst->Q10_diff = ml->data+7*pnodecount;
        inst->Q10_channel = ml->data+8*pnodecount;
        inst->fix_celsius = ml->data+9*pnodecount;
        inst->ic = ml->data+10*pnodecount;
        inst->n_inf = ml->data+11*pnodecount;
        inst->tau_n = ml->data+12*pnodecount;
        inst->g = ml->data+13*pnodecount;
        inst->gbar_Q10 = ml->data+14*pnodecount;
        inst->n = ml->data+15*pnodecount;
        inst->ek = ml->data+16*pnodecount;
        inst->Dn = ml->data+17*pnodecount;
        inst->ik = ml->data+18*pnodecount;
    ...
    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_KV_Instance const* inst) {
        ...
        tmp.Aalpha_n = cnrn_target_deviceptr(tmp.Aalpha_n);
        tmp.Kalpha_n = cnrn_target_deviceptr(tmp.Kalpha_n);
        tmp.V0alpha_n = cnrn_target_deviceptr(tmp.V0alpha_n);
        tmp.Abeta_n = cnrn_target_deviceptr(tmp.Abeta_n);
        tmp.Kbeta_n = cnrn_target_deviceptr(tmp.Kbeta_n);
        tmp.V0beta_n = cnrn_target_deviceptr(tmp.V0beta_n);
        ...
        tmp.n_inf = cnrn_target_deviceptr(tmp.n_inf);
        tmp.tau_n = cnrn_target_deviceptr(tmp.tau_n);
    ...
    static void nrn_alloc_GRANULE_KV(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline static double alp_n_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double bet_n_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double linoid_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lx, double _ly);
    inline static int rate_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);


    struct functor_GRANULE_KV_0 {
        NrnThread* nt;
        GRANULE_KV_Instance* inst;
        int id;
        int pnodecount;
        double v;
        const Datum* indexes;
        ...
        void initialize() {
            rate_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v, v);
            old_n = inst->n[id];
        }

        functor_GRANULE_KV_0(NrnThread* nt, GRANULE_KV_Instance* inst, int id, int pnodecount, double v, const Datum* indexes, double* data, ThreadDatum* thread)
        ...
        void operator()(const Eigen::Matrix<double, 1, 1>& nmodl_eigen_xm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_dxm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_fm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_jm) const {
            ...
            nmodl_eigen_f[static_cast<int>(0)] = (nt->_dt * ( -nmodl_eigen_x[static_cast<int>(0)] + inst->n_inf[id]) + inst->tau_n[id] * ( -nmodl_eigen_x[static_cast<int>(0)] + old_n)) / (nt->_dt * inst->tau_n[id]);
            nmodl_eigen_j[static_cast<int>(0)] = ( -nt->_dt - inst->tau_n[id]) / (nt->_dt * inst->tau_n[id]);
        }
    ...
    inline int f_rate_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        int ret_f_rate = 0;
        double a_n, b_n, alp_n_in_0, bet_n_in_0;
        {
            double Q10, linoid_in_0, _lv_in_0;
            _lv_in_0 = _lv;
            Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 6.3) / 10.0));
            {
                double _lx_in_0, _ly_in_0;
                _lx_in_0 = _lv_in_0 - inst->V0alpha_n[id];
                _ly_in_0 = inst->Kalpha_n[id];
            ...
            alp_n_in_0 = Q10 * inst->Aalpha_n[id] * linoid_in_0;
        }
        a_n = alp_n_in_0;
        {
            double Q10, _lv_in_1;
            _lv_in_1 = _lv;
            Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 6.3) / 10.0));
            bet_n_in_0 = Q10 * inst->Abeta_n[id] * exp((_lv_in_1 - inst->V0beta_n[id]) / inst->Kbeta_n[id]);
        }
        b_n = bet_n_in_0;
        inst->tau_n[id] = 1.0 / (a_n + b_n);
        inst->n_inf[id] = a_n / (a_n + b_n);
        return ret_f_rate;
    }


    void update_table_rate_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        ...
        if (make_table) {
            ...
            for (std::size_t i = 0; i < 13001; x += dx, i++) {
                f_rate_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v, x);
    ...
    inline static int rate_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv){
        if (inst->global->usetable == 0) {
            f_rate_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v, _lv);
            return 0;
        }
    ...
    inline double alp_n_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_alp_n = 0.0;
        double Q10, linoid_in_0;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 6.3) / 10.0));
        {
            double _lx_in_0, _ly_in_0;
            _lx_in_0 = _lv - inst->V0alpha_n[id];
            _ly_in_0 = inst->Kalpha_n[id];
            if (fabs(_lx_in_0 / _ly_in_0) < 1e-6) {
                linoid_in_0 = _ly_in_0 * (1.0 - _lx_in_0 / _ly_in_0 / 2.0);
            } else {
        ...
        ret_alp_n = Q10 * inst->Aalpha_n[id] * linoid_in_0;
        return ret_alp_n;
    }


    inline double bet_n_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_bet_n = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 6.3) / 10.0));
        ret_bet_n = Q10 * inst->Abeta_n[id] * exp((_lv - inst->V0beta_n[id]) / inst->Kbeta_n[id]);
        return ret_bet_n;
    }


    inline double linoid_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lx, double _ly) {
    ...
    /** initialize channel */
    void nrn_init_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            ...
            if (nt->compute_gpu) {
                nrn_pragma_acc(update device (GRANULE_KV_global))
                nrn_pragma_omp(target update to(GRANULE_KV_global))
            }
    ...
    inline double nrn_current_GRANULE_KV(int id, int pnodecount, GRANULE_KV_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->g[id] = inst->gbar_Q10[id] * inst->n[id] * inst->n[id] * inst->n[id] * inst->n[id];
    ...
    /** update current */
    void nrn_cur_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                #endif
                inst->ek[id] = inst->ion_ek[indexes[0*pnodecount + id]];
                double g = nrn_current_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v+0.001);
                double dik = inst->ik[id];
                double rhs = nrn_current_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v);
                g = (g-rhs)/0.001;
    ...
    /** update state */
    void nrn_state_GRANULE_KV(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                nmodl_eigen_x[static_cast<int>(0)] = inst->n[id];
                // call newton solver
                functor_GRANULE_KV_0 newton_functor(nt, inst, id, pnodecount, v, indexes, data, thread);
                newton_functor.initialize();
                int newton_iterations = nmodl::newton::newton_solver(nmodl_eigen_xm, newton_functor);
                if (newton_iterations < 0) assert(false && "Newton solver did not converge!");
                inst->n[id] = nmodl_eigen_x[static_cast<int>(0)];
    ...
    static void check_table_thread_GRANULE_KV (int id, int pnodecount, double* data, Datum* indexes, ThreadDatum* thread, NrnThread* nt, Memb_list* ml, int tml_id) {
        setup_instance(nt, ml);
        auto* const inst = static_cast<GRANULE_KV_Instance*>(ml->instance);
        double v = 0;
        update_table_rate_GRANULE_KV(id, pnodecount, inst, data, indexes, thread, nt, v);
    }
    ...
    void _GRANULE_KV_reg() {
        ...
        int mech_type = nrn_get_mechtype("GRANULE_KV");
        GRANULE_KV_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_GRANULE_KV, nrn_cur_GRANULE_KV, nullptr, nrn_state_GRANULE_KV, nrn_init_GRANULE_KV, nrn_private_constructor_GRANULE_KV, nrn_private_destructor_GRANULE_KV, first_pointer_var_index(), 1);
        GRANULE_KV_global.k_type = nrn_get_mechtype("k_ion");

        _nrn_thread_table_reg(mech_type, check_table_thread_GRANULE_KV);
