/*********************************************************
Model Name      : GRANULE_NA
Filename        : GRANULE_NA.mod
NMODL Version   : 7.7.0
Vectorized      : true
Threadsafe      : true
...
namespace coreneuron {
    ...
    struct GRANULE_NA_Store {
        ...
        double t_h_inf[13001]{};
        double t_tau_h[13001]{};
    };
    static_assert(std::is_trivially_copy_constructible_v<GRANULE_NA_Store>);
    static_assert(std::is_trivially_move_constructible_v<GRANULE_NA_Store>);
    static_assert(std::is_trivially_copy_assignable_v<GRANULE_NA_Store>);
    static_assert(std::is_trivially_move_assignable_v<GRANULE_NA_Store>);
    static_assert(std::is_trivially_destructible_v<GRANULE_NA_Store>);
    static GRANULE_NA_Store GRANULE_NA_global;


    /** all mechanism instance variables and global variables */
    struct GRANULE_NA_Instance  {
        ...
        const double* Aalpha_h{};
        const double* Kalpha_h{};
        const double* V0alpha_h{};
        const double* Abeta_h{};
        const double* Kbeta_h{};
        const double* V0beta_h{};
        const double* Q10_diff{};
        const double* Q10_channel_alp_m{};
        const double* Q10_channel_bet_m{};
        const double* Q10_channel_alp_h{};
        const double* Q10_channel_bet_h{};
        const double* gbar{};
        ...
        double* ion_ina{};
        double* ion_dinadv{};
        GRANULE_NA_Store* global{&GRANULE_NA_global};
    };


    /** connect global (scalar) variables to hoc -- */
    static DoubScal hoc_scalar_double[] = {
        {"usetable_GRANULE_NA", &GRANULE_NA_global.usetable},
        {nullptr, nullptr}
    };
    ...
    static inline int get_mech_type() {
        return GRANULE_NA_global.mech_type;
    }
    ...
    static void nrn_private_constructor_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new GRANULE_NA_Instance{};
        assert(inst->global == &GRANULE_NA_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(GRANULE_NA_Store);
    }

    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_NA_Instance const* inst);
    static inline void delete_instance_from_device(GRANULE_NA_Instance* inst);
    // Deallocate the instance structure
    static void nrn_private_destructor_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<GRANULE_NA_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &GRANULE_NA_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(GRANULE_NA_Store));
    ...
    static inline void setup_instance(NrnThread* nt, Memb_list* ml) {
        ...
        inst->Aalpha_h = ml->data+6*pnodecount;
        inst->Kalpha_h = ml->data+7*pnodecount;
        inst->V0alpha_h = ml->data+8*pnodecount;
        inst->Abeta_h = ml->data+9*pnodecount;
        inst->Kbeta_h = ml->data+10*pnodecount;
        inst->V0beta_h = ml->data+11*pnodecount;
        inst->Q10_diff = ml->data+12*pnodecount;
        inst->Q10_channel_alp_m = ml->data+13*pnodecount;
        inst->Q10_channel_bet_m = ml->data+14*pnodecount;
        inst->Q10_channel_alp_h = ml->data+15*pnodecount;
        inst->Q10_channel_bet_h = ml->data+16*pnodecount;
        inst->gbar = ml->data+17*pnodecount;
        inst->fix_celsius = ml->data+18*pnodecount;
        inst->ic = ml->data+19*pnodecount;
        inst->m_inf = ml->data+20*pnodecount;
        inst->h_inf = ml->data+21*pnodecount;
        inst->tau_m = ml->data+22*pnodecount;
        inst->tau_h = ml->data+23*pnodecount;
        inst->g = ml->data+24*pnodecount;
        inst->gbar_Q10 = ml->data+25*pnodecount;
        inst->m = ml->data+26*pnodecount;
        inst->h = ml->data+27*pnodecount;
        inst->ena = ml->data+28*pnodecount;
        inst->Dm = ml->data+29*pnodecount;
        inst->Dh = ml->data+30*pnodecount;
        inst->ina = ml->data+31*pnodecount;
    ...
    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_NA_Instance const* inst) {
        ...
        tmp.Aalpha_h = cnrn_target_deviceptr(tmp.Aalpha_h);
        tmp.Kalpha_h = cnrn_target_deviceptr(tmp.Kalpha_h);
        tmp.V0alpha_h = cnrn_target_deviceptr(tmp.V0alpha_h);
        tmp.Abeta_h = cnrn_target_deviceptr(tmp.Abeta_h);
        tmp.Kbeta_h = cnrn_target_deviceptr(tmp.Kbeta_h);
        tmp.V0beta_h = cnrn_target_deviceptr(tmp.V0beta_h);
        tmp.Q10_diff = cnrn_target_deviceptr(tmp.Q10_diff);
        tmp.Q10_channel_alp_m = cnrn_target_deviceptr(tmp.Q10_channel_alp_m);
        tmp.Q10_channel_bet_m = cnrn_target_deviceptr(tmp.Q10_channel_bet_m);
        tmp.Q10_channel_alp_h = cnrn_target_deviceptr(tmp.Q10_channel_alp_h);
        tmp.Q10_channel_bet_h = cnrn_target_deviceptr(tmp.Q10_channel_bet_h);
        ...
        tmp.h_inf = cnrn_target_deviceptr(tmp.h_inf);
        tmp.tau_m = cnrn_target_deviceptr(tmp.tau_m);
        tmp.tau_h = cnrn_target_deviceptr(tmp.tau_h);
    ...
    static void nrn_alloc_GRANULE_NA(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline static double alp_m_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double bet_m_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double alp_h_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double bet_h_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double linoid_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lx, double _ly);
    inline static int rate_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);


    struct functor_GRANULE_NA_0 {
        NrnThread* nt;
        GRANULE_NA_Instance* inst;
        int id;
        int pnodecount;
        double v;
        const Datum* indexes;
        ...
        void initialize() {
            rate_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v, v);
            old_m = inst->m[id];
            old_h = inst->h[id];
        }

        functor_GRANULE_NA_0(NrnThread* nt, GRANULE_NA_Instance* inst, int id, int pnodecount, double v, const Datum* indexes, double* data, ThreadDatum* thread)
        ...
        void operator()(const Eigen::Matrix<double, 2, 1>& nmodl_eigen_xm, Eigen::Matrix<double, 2, 1>& nmodl_eigen_dxm, Eigen::Matrix<double, 2, 1>& nmodl_eigen_fm, Eigen::Matrix<double, 2, 2>& nmodl_eigen_jm) const {
            ...
            nmodl_eigen_f[static_cast<int>(1)] = (nt->_dt * ( -nmodl_eigen_x[static_cast<int>(1)] + inst->h_inf[id]) + inst->tau_h[id] * ( -nmodl_eigen_x[static_cast<int>(1)] + old_h)) / (nt->_dt * inst->tau_h[id]);
            nmodl_eigen_j[static_cast<int>(1)] = 0.0;
            nmodl_eigen_j[static_cast<int>(3)] = ( -nt->_dt - inst->tau_h[id]) / (nt->_dt * inst->tau_h[id]);
        }
    ...
    inline int f_rate_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        int ret_f_rate = 0;
        double a_m, b_m, a_h, b_h, alp_m_in_0, bet_m_in_0, alp_h_in_0, bet_h_in_0;
        {
            double Q10, linoid_in_0, _lv_in_0;
            _lv_in_0 = _lv;
            Q10 = pow(inst->Q10_channel_alp_m[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            {
        ...
        a_m = alp_m_in_0;
        {
            double Q10, _lv_in_1;
            _lv_in_1 = _lv;
            Q10 = pow(inst->Q10_channel_bet_m[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            bet_m_in_0 = Q10 * inst->Abeta_m[id] * exp((_lv_in_1 - inst->V0beta_m[id]) / inst->Kbeta_m[id]);
        }
        b_m = bet_m_in_0;
        {
            double Q10, _lv_in_2;
            _lv_in_2 = _lv;
            Q10 = pow(inst->Q10_channel_alp_h[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            alp_h_in_0 = Q10 * inst->Aalpha_h[id] * exp((_lv_in_2 - inst->V0alpha_h[id]) / inst->Kalpha_h[id]);
        }
        a_h = alp_h_in_0;
        {
            double Q10, _lv_in_3;
            _lv_in_3 = _lv;
            Q10 = pow(inst->Q10_channel_bet_h[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            bet_h_in_0 = Q10 * inst->Abeta_h[id] / (1.0 + exp((_lv_in_3 - inst->V0beta_h[id]) / inst->Kbeta_h[id]));
        }
        b_h = bet_h_in_0;
        inst->m_inf[id] = a_m / (a_m + b_m);
        inst->tau_m[id] = 1.0 / (a_m + b_m);
        inst->h_inf[id] = a_h / (a_h + b_h);
        inst->tau_h[id] = 1.0 / (a_h + b_h);
        return ret_f_rate;
    }


    void update_table_rate_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        ...
        static double save_Aalpha_h;
        static double save_Kalpha_h;
        static double save_V0alpha_h;
        static double save_Abeta_h;
        static double save_Kbeta_h;
        static double save_V0beta_h;
        static double save_fix_celsius;
        static double save_Q10_channel_bet_h;
        static double save_Q10_channel_alp_h;
        static double save_Q10_channel_bet_m;
        static double save_Q10_channel_alp_m;
        ...
        if (save_Aalpha_h != inst->Aalpha_h[id]) {
            make_table = true;
        }
        if (save_Kalpha_h != inst->Kalpha_h[id]) {
            make_table = true;
        }
        if (save_V0alpha_h != inst->V0alpha_h[id]) {
            make_table = true;
        }
        if (save_Abeta_h != inst->Abeta_h[id]) {
            make_table = true;
        }
        if (save_Kbeta_h != inst->Kbeta_h[id]) {
            make_table = true;
        }
        if (save_V0beta_h != inst->V0beta_h[id]) {
            make_table = true;
        }
        if (save_fix_celsius != inst->fix_celsius[id]) {
            make_table = true;
        }
        if (save_Q10_channel_bet_h != inst->Q10_channel_bet_h[id]) {
            make_table = true;
        }
        if (save_Q10_channel_alp_h != inst->Q10_channel_alp_h[id]) {
            make_table = true;
        }
        if (save_Q10_channel_bet_m != inst->Q10_channel_bet_m[id]) {
            make_table = true;
        }
        if (save_Q10_channel_alp_m != inst->Q10_channel_alp_m[id]) {
            make_table = true;
        }
        if (make_table) {
            make_table = false;
            ...
            for (std::size_t i = 0; i < 13001; x += dx, i++) {
                f_rate_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v, x);
                inst->global->t_m_inf[i] = inst->m_inf[id];
                inst->global->t_tau_m[i] = inst->tau_m[id];
                inst->global->t_h_inf[i] = inst->h_inf[id];
                inst->global->t_tau_h[i] = inst->tau_h[id];
            }
            ...
            save_Aalpha_h = inst->Aalpha_h[id];
            save_Kalpha_h = inst->Kalpha_h[id];
            save_V0alpha_h = inst->V0alpha_h[id];
            save_Abeta_h = inst->Abeta_h[id];
            save_Kbeta_h = inst->Kbeta_h[id];
            save_V0beta_h = inst->V0beta_h[id];
            save_fix_celsius = inst->fix_celsius[id];
            save_Q10_channel_bet_h = inst->Q10_channel_bet_h[id];
            save_Q10_channel_alp_h = inst->Q10_channel_alp_h[id];
            save_Q10_channel_bet_m = inst->Q10_channel_bet_m[id];
            save_Q10_channel_alp_m = inst->Q10_channel_alp_m[id];
        }
    }
    ...
    inline static int rate_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv){
        if (inst->global->usetable == 0) {
            f_rate_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v, _lv);
            return 0;
        }
        ...
        if (xi <= 0. || xi >= 13000.) {
            ...
            inst->h_inf[id] = inst->global->t_h_inf[index];
            inst->tau_h[id] = inst->global->t_tau_h[index];
            return 0;
        }
        int i = int(xi);
        ...
        inst->h_inf[id] = inst->global->t_h_inf[i] + theta*(inst->global->t_h_inf[i+1]-inst->global->t_h_inf[i]);
        inst->tau_h[id] = inst->global->t_tau_h[i] + theta*(inst->global->t_tau_h[i+1]-inst->global->t_tau_h[i]);
        return 0;
    }


    inline double alp_m_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
    ...
    inline double bet_m_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_bet_m = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel_bet_m[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_bet_m = Q10 * inst->Abeta_m[id] * exp((_lv - inst->V0beta_m[id]) / inst->Kbeta_m[id]);
        return ret_bet_m;
    }


    inline double alp_h_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_alp_h = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel_alp_h[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_alp_h = Q10 * inst->Aalpha_h[id] * exp((_lv - inst->V0alpha_h[id]) / inst->Kalpha_h[id]);
        return ret_alp_h;
    }


    inline double bet_h_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_bet_h = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel_bet_h[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_bet_h = Q10 * inst->Abeta_h[id] / (1.0 + exp((_lv - inst->V0beta_h[id]) / inst->Kbeta_h[id]));
        return ret_bet_h;
    }


    inline double linoid_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lx, double _ly) {
    ...
    /** initialize channel */
    void nrn_init_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            ...
            if (nt->compute_gpu) {
                nrn_pragma_acc(update device (GRANULE_NA_global))
                nrn_pragma_omp(target update to(GRANULE_NA_global))
            }
    ...
    inline double nrn_current_GRANULE_NA(int id, int pnodecount, GRANULE_NA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->g[id] = inst->gbar_Q10[id] * inst->m[id] * inst->m[id] * inst->m[id] * inst->h[id];
    ...
    /** update current */
    void nrn_cur_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                #endif
                inst->ena[id] = inst->ion_ena[indexes[0*pnodecount + id]];
                double g = nrn_current_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v+0.001);
                double dina = inst->ina[id];
                double rhs = nrn_current_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v);
                g = (g-rhs)/0.001;
    ...
    /** update state */
    void nrn_state_GRANULE_NA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                nmodl_eigen_x[static_cast<int>(1)] = inst->h[id];
                // call newton solver
                functor_GRANULE_NA_0 newton_functor(nt, inst, id, pnodecount, v, indexes, data, thread);
                newton_functor.initialize();
                int newton_iterations = nmodl::newton::newton_solver(nmodl_eigen_xm, newton_functor);
                if (newton_iterations < 0) assert(false && "Newton solver did not converge!");
                inst->m[id] = nmodl_eigen_x[static_cast<int>(0)];
                inst->h[id] = nmodl_eigen_x[static_cast<int>(1)];
    ...
    static void check_table_thread_GRANULE_NA (int id, int pnodecount, double* data, Datum* indexes, ThreadDatum* thread, NrnThread* nt, Memb_list* ml, int tml_id) {
        setup_instance(nt, ml);
        auto* const inst = static_cast<GRANULE_NA_Instance*>(ml->instance);
        double v = 0;
        update_table_rate_GRANULE_NA(id, pnodecount, inst, data, indexes, thread, nt, v);
    }
    ...
    void _GRANULE_NA_reg() {
        ...
        int mech_type = nrn_get_mechtype("GRANULE_NA");
        GRANULE_NA_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_GRANULE_NA, nrn_cur_GRANULE_NA, nullptr, nrn_state_GRANULE_NA, nrn_init_GRANULE_NA, nrn_private_constructor_GRANULE_NA, nrn_private_destructor_GRANULE_NA, first_pointer_var_index(), 1);
        GRANULE_NA_global.na_type = nrn_get_mechtype("na_ion");

        _nrn_thread_table_reg(mech_type, check_table_thread_GRANULE_NA);
