/*********************************************************
Model Name      : GRANULE_CA
Filename        : GRANULE_CA.mod
NMODL Version   : 7.7.0
Vectorized      : true
Threadsafe      : true
...
namespace coreneuron {
    ...
    static_assert(std::is_trivially_copy_constructible_v<GRANULE_CA_Store>);
    static_assert(std::is_trivially_move_constructible_v<GRANULE_CA_Store>);
    static_assert(std::is_trivially_copy_assignable_v<GRANULE_CA_Store>);
    static_assert(std::is_trivially_move_assignable_v<GRANULE_CA_Store>);
    static_assert(std::is_trivially_destructible_v<GRANULE_CA_Store>);
    static GRANULE_CA_Store GRANULE_CA_global;


    /** all mechanism instance variables and global variables */
    struct GRANULE_CA_Instance  {
        ...
        double* ion_ica{};
        double* ion_dicadv{};
        GRANULE_CA_Store* global{&GRANULE_CA_global};
    };


    /** connect global (scalar) variables to hoc -- */
    static DoubScal hoc_scalar_double[] = {
        {"usetable_GRANULE_CA", &GRANULE_CA_global.usetable},
        {nullptr, nullptr}
    };
    ...
    static inline int get_mech_type() {
        return GRANULE_CA_global.mech_type;
    }
    ...
    static void nrn_private_constructor_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new GRANULE_CA_Instance{};
        assert(inst->global == &GRANULE_CA_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(GRANULE_CA_Store);
    }

    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_CA_Instance const* inst);
    static inline void delete_instance_from_device(GRANULE_CA_Instance* inst);
    // Deallocate the instance structure
    static void nrn_private_destructor_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<GRANULE_CA_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &GRANULE_CA_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(GRANULE_CA_Store));
    ...
    static inline void setup_instance(NrnThread* nt, Memb_list* ml) {
        ...
        inst->Q10_channel = ml->data+13*pnodecount;
        inst->gbar = ml->data+14*pnodecount;
        inst->fix_celsius = ml->data+15*pnodecount;
        inst->ica = ml->data+16*pnodecount;
        inst->ic = ml->data+17*pnodecount;
        inst->s_inf = ml->data+18*pnodecount;
        inst->u_inf = ml->data+19*pnodecount;
        inst->tau_s = ml->data+20*pnodecount;
        inst->tau_u = ml->data+21*pnodecount;
        inst->g = ml->data+22*pnodecount;
        inst->gbar_Q10 = ml->data+23*pnodecount;
        inst->s = ml->data+24*pnodecount;
        inst->u = ml->data+25*pnodecount;
        inst->eca = ml->data+26*pnodecount;
        inst->Ds = ml->data+27*pnodecount;
        inst->Du = ml->data+28*pnodecount;
        inst->v_unused = ml->data+29*pnodecount;
        inst->g_unused = ml->data+30*pnodecount;
    ...
    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, GRANULE_CA_Instance const* inst) {
        ...
        tmp.Aalpha_u = cnrn_target_deviceptr(tmp.Aalpha_u);
        tmp.Kalpha_u = cnrn_target_deviceptr(tmp.Kalpha_u);
        tmp.V0alpha_u = cnrn_target_deviceptr(tmp.V0alpha_u);
        tmp.Abeta_u = cnrn_target_deviceptr(tmp.Abeta_u);
        tmp.Kbeta_u = cnrn_target_deviceptr(tmp.Kbeta_u);
        tmp.V0beta_u = cnrn_target_deviceptr(tmp.V0beta_u);
    ...
    static void nrn_alloc_GRANULE_CA(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline static double alp_s_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double bet_s_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double alp_u_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static double bet_u_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);
    inline static int rate_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv);


    struct functor_GRANULE_CA_0 {
        NrnThread* nt;
        GRANULE_CA_Instance* inst;
        int id;
        int pnodecount;
        double v;
        const Datum* indexes;
        ...
        void initialize() {
            rate_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v, v);
            old_s = inst->s[id];
            old_u = inst->u[id];
        }

        functor_GRANULE_CA_0(NrnThread* nt, GRANULE_CA_Instance* inst, int id, int pnodecount, double v, const Datum* indexes, double* data, ThreadDatum* thread)
        ...
        void operator()(const Eigen::Matrix<double, 2, 1>& nmodl_eigen_xm, Eigen::Matrix<double, 2, 1>& nmodl_eigen_dxm, Eigen::Matrix<double, 2, 1>& nmodl_eigen_fm, Eigen::Matrix<double, 2, 2>& nmodl_eigen_jm) const {
            ...
            nmodl_eigen_f[static_cast<int>(1)] = (nt->_dt * ( -nmodl_eigen_x[static_cast<int>(1)] + inst->u_inf[id]) + inst->tau_u[id] * ( -nmodl_eigen_x[static_cast<int>(1)] + old_u)) / (nt->_dt * inst->tau_u[id]);
            nmodl_eigen_j[static_cast<int>(1)] = 0.0;
            nmodl_eigen_j[static_cast<int>(3)] = ( -nt->_dt - inst->tau_u[id]) / (nt->_dt * inst->tau_u[id]);
        }
    ...
    inline int f_rate_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        int ret_f_rate = 0;
        double a_s, b_s, a_u, b_u, alp_s_in_0, bet_s_in_0, alp_u_in_0, bet_u_in_0;
        {
            ...
            alp_s_in_0 = Q10 * inst->Aalpha_s[id] * exp((_lv_in_0 - inst->V0alpha_s[id]) / inst->Kalpha_s[id]);
        }
        a_s = alp_s_in_0;
        {
            double Q10, _lv_in_1;
            _lv_in_1 = _lv;
            Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            bet_s_in_0 = Q10 * inst->Abeta_s[id] * exp((_lv_in_1 - inst->V0beta_s[id]) / inst->Kbeta_s[id]);
        }
        b_s = bet_s_in_0;
        {
            double Q10, _lv_in_2;
            _lv_in_2 = _lv;
            Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            alp_u_in_0 = Q10 * inst->Aalpha_u[id] * exp((_lv_in_2 - inst->V0alpha_u[id]) / inst->Kalpha_u[id]);
        }
        a_u = alp_u_in_0;
        {
            double Q10, _lv_in_3;
            _lv_in_3 = _lv;
            Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
            bet_u_in_0 = Q10 * inst->Abeta_u[id] * exp((_lv_in_3 - inst->V0beta_u[id]) / inst->Kbeta_u[id]);
        }
        b_u = bet_u_in_0;
    ...
    void update_table_rate_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        ...
        if (make_table) {
            ...
            for (std::size_t i = 0; i < 13001; x += dx, i++) {
                f_rate_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v, x);
    ...
    inline static int rate_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv){
        if (inst->global->usetable == 0) {
            f_rate_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v, _lv);
            return 0;
        }
    ...
    inline double alp_s_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_alp_s = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_alp_s = Q10 * inst->Aalpha_s[id] * exp((_lv - inst->V0alpha_s[id]) / inst->Kalpha_s[id]);
        return ret_alp_s;
    }


    inline double bet_s_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_bet_s = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_bet_s = Q10 * inst->Abeta_s[id] * exp((_lv - inst->V0beta_s[id]) / inst->Kbeta_s[id]);
        return ret_bet_s;
    }


    inline double alp_u_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_alp_u = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_alp_u = Q10 * inst->Aalpha_u[id] * exp((_lv - inst->V0alpha_u[id]) / inst->Kalpha_u[id]);
        return ret_alp_u;
    }


    inline double bet_u_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v, double _lv) {
        double ret_bet_u = 0.0;
        double Q10;
        Q10 = pow(inst->Q10_channel[id], ((inst->fix_celsius[id] - 20.0) / 10.0));
        ret_bet_u = Q10 * inst->Abeta_u[id] * exp((_lv - inst->V0beta_u[id]) / inst->Kbeta_u[id]);
        return ret_bet_u;
    }


    /** initialize channel */
    void nrn_init_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            ...
            if (nt->compute_gpu) {
                nrn_pragma_acc(update device (GRANULE_CA_global))
                nrn_pragma_omp(target update to(GRANULE_CA_global))
            }
    ...
    inline double nrn_current_GRANULE_CA(int id, int pnodecount, GRANULE_CA_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->g[id] = inst->gbar_Q10[id] * inst->s[id] * inst->s[id] * inst->u[id];
        inst->ica[id] = inst->g[id] * (v - inst->eca[id]);
        inst->ic[id] = inst->ica[id];
        current += inst->ica[id];
        return current;
    }
    ...
    /** update current */
    void nrn_cur_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                #endif
                inst->eca[id] = inst->ion_eca[indexes[0*pnodecount + id]];
                double g = nrn_current_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v+0.001);
                double dica = inst->ica[id];
                double rhs = nrn_current_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v);
                g = (g-rhs)/0.001;
    ...
    /** update state */
    void nrn_state_GRANULE_CA(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                nmodl_eigen_x[static_cast<int>(1)] = inst->u[id];
                // call newton solver
                functor_GRANULE_CA_0 newton_functor(nt, inst, id, pnodecount, v, indexes, data, thread);
                newton_functor.initialize();
                int newton_iterations = nmodl::newton::newton_solver(nmodl_eigen_xm, newton_functor);
                if (newton_iterations < 0) assert(false && "Newton solver did not converge!");
                inst->s[id] = nmodl_eigen_x[static_cast<int>(0)];
                inst->u[id] = nmodl_eigen_x[static_cast<int>(1)];
    ...
    static void check_table_thread_GRANULE_CA (int id, int pnodecount, double* data, Datum* indexes, ThreadDatum* thread, NrnThread* nt, Memb_list* ml, int tml_id) {
        setup_instance(nt, ml);
        auto* const inst = static_cast<GRANULE_CA_Instance*>(ml->instance);
        double v = 0;
        update_table_rate_GRANULE_CA(id, pnodecount, inst, data, indexes, thread, nt, v);
    }
    ...
    void _GRANULE_CA_reg() {
        ...
        int mech_type = nrn_get_mechtype("GRANULE_CA");
        GRANULE_CA_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_GRANULE_CA, nrn_cur_GRANULE_CA, nullptr, nrn_state_GRANULE_CA, nrn_init_GRANULE_CA, nrn_private_constructor_GRANULE_CA, nrn_private_destructor_GRANULE_CA, first_pointer_var_index(), 1);
        GRANULE_CA_global.ca_type = nrn_get_mechtype("ca_ion");

        _nrn_thread_table_reg(mech_type, check_table_thread_GRANULE_CA);
