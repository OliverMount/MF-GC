...
# ==========================
MAX_CPUS=20    # Maximum number of CPUs and tasks to test
TSTOP=1000    # Simulation stop time
RUNTYPE="coreneuron_cpu"  # ["coreneuron_gpu", "coreneuron_cpu", "neuron_serial", "neuron_parallel"]
...
for iteration in $(seq 1 $ITERATIONS); do
    ...
    # Run the simulations
    # ==========================
    for g in $(seq $MAX_CPUS $MAX_CPUS); do
        t=$g  # Match tasks to GPUs

	ERR_FILE="../results/b_${t}_1M.err"
        OUT_FILE="../results/b_${t}_1M.out"

        OUTFILE=$OUT_FILE
        ERRFILE=$ERR_FILE

        echo -e "\nStarting simulation: CPUs=$g, Rank(s)=$t, tstop=$TSTOP"
        start=$(date +%s)
        echo "Start time: $(date -d @$start '+%Y-%m-%d %H:%M:%S')"

        apptainer exec --bind $LOCAL_PROJECT_DIR:$APPTAINER_MOUNT --pwd $APPTAINER_MOUNT/src "$APPTAINER_IMAGE" \
        ...
        # Save results as CSV
        # ==========================
        echo "${sys},CORENEURON_CPU,${g},${t},${solver_time},${total_runtime_fmt} (${total_runtime}s)" >> $CSV_FILE
