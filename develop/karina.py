...
db_file='../connectivity/mf_grc_scaled_2.db'
    ...
    with sqlite3.connect(db_file) as conn:
        ...
        if nhost > 1:
            for ran in range(nhost):
                print_status(f'rank {rank} ==> creating extra syanpse table')
...
if nhost>1:
    with sqlite3.connect(os.path.join(db_rank_folder, f'synapse_rank{rank}.db')) as conn:
        ...
        print_status(f"DB path from rank ==> {rank} is "+os.path.join(db_rank_folder, f'synapse_rank{rank}.db'))
